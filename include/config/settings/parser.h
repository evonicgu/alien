...
namespace alien::config::settings {
    ...
    private:
        ...
        std::pair<TTp*, VTp*> check_value(bool& type_err, std::shared_ptr<value>& setting_ptr) {
        ...
        void set_value(const util::u8string& name, std::shared_ptr<value>& set_value) {
        ...
        void code_block() {
            ...
            switch (block->loc) {
                case code_token::location::TOP:
                    configuration.code_top.push_back(std::move(block->code));
                    break;
                case code_token::location::DEFAULT:
                    configuration.code.push_back(std::move(block->code));
                    break;
                case code_token::location::CONTENT:
                    configuration.code_content.push_back(std::move(block->code));
                    break;
            }
